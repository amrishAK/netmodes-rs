...
fn main() {
    ...
    println!(
        ...
        tcp_server_handler.config().host,
        tcp_server_handler.config().port
    );

    let tcp_server_handler = tcp_server_handler
        .into_listening()
        .expect("Failed to initialize TCP server");
    println!("TCP server has been initialized and is accepting connections");

    let on_message_handler: OnMessageHandler = Arc::new(
        |client_handler: &TcpClientSession, _context_handler: ContextHandler, data: &[u8]| {
            client_handler
                .reply(data)
                .expect("Failed to send reply to client");
